...
// Consistent gradient per faculty id
...
const STATUS = {
  present: {
    ...
    activeBg: "#059669", activeText: "#fff",
    dot: "#10b981", badge: "#d1fae5",
    icon: (
  ...
  leave: {
    ...
    activeBg: "#d97706", activeText: "#fff",
    dot: "#f59e0b", badge: "#fef3c7",
    icon: (
      <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  unmarked: {
    ...
    activeBg: "#94a3b8", activeText: "#fff",
    dot: "#cbd5e1", badge: "#f1f5f9",
    icon: (
      <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const StatsCard = memo(({ label, value, color, icon }) => {
  ...
  return (
    <div style={{
      background: p.bg, borderRadius: 16, padding: "18px 20px",
      border: `1.5px solid ${p.accent}22`, position: "relative", overflow: "hidden",
    }}>
...
// Simple distinct SVG icons — no circle wrappers, just the symbol itself
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "12px 16px", borderTop: "1px solid #f1f5f9", display: "flex", gap: 8 }}>
          ...
          <button
            onClick={() => onSave(faculty.id, selected)}
            disabled={saving}
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              background: "#eef2ff", color: "#4f46e5", fontSize: 11, fontWeight: 600, flexShrink: 0,
            }}
          >
    ...
    <div style={{ padding: "14px 20px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", gap: 14, background: "#fff" }}>
      <div style={{ position: "relative", flexShrink: 0 }}>
        ...
        <div style={{
          ...
          borderRadius: "50%", background: cfg.dot, border: "2px solid #fff",
        }} />
      </div>
        ...
        {faculty.subjects?.length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 6 }}>
            ...
            {faculty.subjects.length > 2 && (
              <span style={{ fontSize: 10, color: "#94a3b8", background: "#f1f5f9", padding: "2px 7px", borderRadius: 5, fontWeight: 600 }}>+{faculty.subjects.length - 2}</span>
            )}
          </div>
        )}
      </div>
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", padding: "24px 24px 48px", fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      ...
      {toast && (
        <div style={{
          ...
          {toast.type === "success"
            ? <PresentIcon size={14} color="#059669" />
            : <AbsentIcon size={14} color="#e11d48" />}
          {toast.message}
        </div>
      )}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        ...
        <button
          onClick={fetchData}
          style={{ display: "flex", alignItems: "center", gap: 7, padding: "9px 16px", borderRadius: 11, border: "1.5px solid #e2e8f0", background: "#fff", color: "#64748b", fontSize: 12, fontWeight: 600, cursor: "pointer" }}
        >
          <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div style={{ display: "flex", gap: 4, padding: 4, background: "#fff", border: "1.5px solid #e2e8f0", borderRadius: 14, width: "fit-content", marginBottom: 24, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
        {[
          { key: "faculty", label: "Faculty & Subjects", emoji: "👥" },
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 14 }}>
            {filteredFaculty.length === 0 ? (
              <div style={{ gridColumn: "1/-1", padding: "64px 0", textAlign: "center", color: "#94a3b8", background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", fontSize: 13 }}>
      ...
      {activeTab === "attendance" && (
        <div>
          ...
          <div style={{ background: "#fff", border: "1.5px solid #e2e8f0", borderRadius: 16, padding: "14px 18px", marginBottom: 18, display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 2, background: "#f1f5f9", borderRadius: 9, padding: 3 }}>
                ...
                <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)}
                  style={{ fontSize: 12, fontWeight: 600, color: "#1e293b", border: "none", background: "transparent", outline: "none", padding: "0 4px" }} />
                <button onClick={() => handleDateChange(1)} style={{ width: 30, height: 30, borderRadius: 7, border: "none", background: "none", cursor: "pointer", color: "#64748b", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>›</button>
              </div>
              ...
              {["present", "absent", "leave"].map(s => {
                ...
                return (
                  <button key={s} onClick={() => handleMarkAll(s)} style={{
                    padding: "5px 11px", borderRadius: 8, border: "none", cursor: "pointer",
                    background: STATUS[s].bg, color: STATUS[s].text, fontSize: 11, fontWeight: 600,
                    display: "flex", alignItems: "center", gap: 5,
                  }}>
            ...
            <div style={{ marginLeft: "auto" }}>
              <button onClick={handleSaveAttendance} disabled={attendanceSaving} style={{ ...btnPrimary, display: "flex", alignItems: "center", gap: 6, opacity: attendanceSaving ? 0.65 : 1 }}>
                {attendanceSaving ? <><SpinIcon /> Saving…</> : (
                  <><svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg> Save Attendance</>
                )}
              </button>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 18 }}>
            ...
            <StatsCard label="Unmarked" value={attendanceStats.unmarked} color="slate" icon={STATUS.unmarked.icon} />
          </div>
              ...
              {filteredAttendance.length === 0 ? (
                ...
                filteredAttendance.map(f => (
                  <AttendanceRow
                    key={f.id}
                    faculty={f}
                    status={attendance[f.id] ?? "unmarked"}
